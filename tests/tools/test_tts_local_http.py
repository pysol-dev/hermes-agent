...
def test_check_tts_requirements_true_when_local_http_sidecar_ready(monkeypatch):
    get = Mock(return_value=_Response(b"{}", status_code=200, content_type="application/json"))
    monkeypatch.setattr("requests.get", get)
    monkeypatch.setattr(
        tts_tool,
        "_load_tts_config",
        lambda: {"provider": "local_http", "local_http": {"base_url": "http://127.0.0.1:8020/v1"}},
    )
...
def test_check_tts_requirements_false_when_local_http_sidecar_unreachable(monkeypatch):
