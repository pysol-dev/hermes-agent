...
class TestVoiceChannelCommands:
    ...
    async def test_join_success(self, runner):
        ...
        assert (
            runner._extract_voice_command(111, 42, "Why aren't you responding?")
    ...
    def test_extract_voice_command_accepts_mid_sentence_vocative_wake(self, runner):
        assert (
            runner._extract_voice_command(
                111,
                42,
                "What do you mean adjust the join follow-up behavior, SIFROT? What do you mean by adjust the follow-up behavior?",
            )
            == "What do you mean by adjust the follow-up behavior?"
        )

    def test_extract_voice_command_accepts_sit_front_wake_variant(self, runner):
        assert (
            runner._extract_voice_command(
                111,
                42,
                "sit front. I mentioned you twice and you still responded. I want you to fix this behavior immediately.",
            )
            == "I mentioned you twice and you still responded. I want you to fix this behavior immediately."
        )

    def test_extract_voice_command_uses_preamble_for_end_vocative(self, runner):
        assert runner._extract_voice_command(111, 42, "can you check logs, Sifrot?") == "can you check logs"

    def test_extract_voice_command_rejects_topical_wake_mentions(self, runner):
        assert runner._extract_voice_command(111, 42, "we were watching a video about Sifrot yesterday") is None
